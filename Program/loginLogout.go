...
func LoginS(logi InfoLogin) {
	...
	var sb SuperBloque
	sbBytes := make([]byte, unsafe.Sizeof(sb))
	...
	initInoArchivo := BytesToInt(sb.InodeStart) + BytesToInt(sb.InodeSize) + 1
	...
	inoArchBytes := make([]byte, unsafe.Sizeof(inodoArchivo))
	...
	for i := range block {
		...
		bloArchBytes := make([]byte, unsafe.Sizeof(bloqueArchivo))
